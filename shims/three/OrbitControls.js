 ...
 *
 * @Modified by Jose Rojas from examples/controls/OrbitControls.js@r106 
 *
 */
...
THREE.OrbitControls = function ( object, domElement, up ) {
	...
	// How far you can pan left and right, upper and lower limits.
	this.minPanLeftRight = - Infinity; // distance units
	this.maxPanLeftRight = Infinity; // distance units

	// How far you can pan up and down, upper and lower limits.
	this.minPanUpDown = - Infinity; // distance units
	this.maxPanUpDown = Infinity; // distance units
	...
	this.update = function () {
		...
		return function update() {
			...
			scope.target.y = Math.max( scope.minPanUpDown, Math.min( scope.maxPanUpDown, scope.target.y ) );
			//clamp panOffset to bounds
			scope.target.x = Math.max( scope.minPanLeftRight, Math.min( scope.maxPanLeftRight, scope.target.x ) );
	...
	this.dispose = function () {
		...
		scope.domElement.removeEventListener( 'contextmenu', scope.onContextMenu, false );
		scope.domElement.removeEventListener( 'mousedown', scope.onMouseDown, false );
		scope.domElement.removeEventListener( 'wheel', scope.onMouseWheel, false );
  ...
  this.STATE = STATE;
	...
	scope.domElement.addEventListener( 'wheel', this.onMouseWheel, false );

	scope.domElement.addEventListener( 'touchstart', this.onTouchStart, false );
	scope.domElement.addEventListener( 'touchend', this.onTouchEnd, false );
	scope.domElement.addEventListener( 'touchmove', this.onTouchMove, false );
